...
class FlutterVendor extends FlutterBaseController
{
    ...
    public function flutter_create_product($request)
    {
        ...
        if ($isSeller) {
			...
			if($request['is_edit'] == true || $request['is_edit'] == 'true') {
            ...
            if (isset($request['image_ids'])) {
				...
				foreach($request['image_ids'] as $imgId) {
					if(substr( $imgId, 0, 4 ) === "http") {
						$imageIds[] = attachment_url_to_postid($imgId);
						continue;
					}
					$imageIds[] = $imgId;
				}
                update_post_meta($post_id, '_product_image_gallery', join(",", $imageIds));
                if (count($request['image_ids']) > 0) {
                    set_post_thumbnail($post_id, $imageIds[0]);
                }
            }
